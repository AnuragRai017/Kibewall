...
function HeroSkeleton() {
  return (
    <section className="relative h-[80vh] min-h-[600px] flex items-center justify-center bg-muted">
...
async function HeroWrapper() {
  ...
  const topWallpapers = wallpapers.slice(0, 10).map((w) => ({ path: w.path }))

  return <HeroSection topWallpapers={topWallpapers} />
}
    ...
    <section className="py-16 px-4 container mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="aspect-[16/9] bg-muted rounded-lg animate-pulse" />
        ))}
      </div>
    </section>
  )
